...
## 10. External Engine API Integration (Added 2026-03-27)

The Governor now supports **external engine APIs** — any third-party platform
that provides an API can be registered and used to replace any internal engine.

### 10.1 External Engine Registration

External engines are stored in the `external_engines` table with:
- **engine_key**: Unique slug (e.g., `ext-openai-scoring`)
- **api_endpoint**: Base URL for the external API
- **auth_type**: `bearer` | `api_key` | `header` | `none`
- **auth_token_encrypted**: AES-256-GCM encrypted credentials
- **health_endpoint**: Optional path for health checks
- **cost_per_operation_usd**: Declared cost per operation
- **replaces_engine**: Which internal engine this can substitute

### 10.2 External Adapter Pattern

The `external-adapter.ts` module provides:
- `callExternalEngine()` — HTTP POST with standardized payload
- `checkExternalHealth()` — Health check via GET
- Circuit breaker protection (3 failures → open)
- Configurable timeout per engine
- Auth header construction (Bearer, API key, custom)
...
### 10.3 Dispatch Enhancement

The Governor Dispatch stage now maintains two caches:
- `swapCache` — Internal engine-to-engine mappings (existing)
- `externalSwapCache` — Engine-to-external-API mappings (new)

When `resolveEngineWithContext()` finds an external swap, it routes
to `callExternalEngine()` instead of the internal registry.

### 10.4 Database Schema (Migration 033)

```
external_engines (NEW TABLE)
├── id UUID PK
├── engine_key TEXT UNIQUE
├── display_name TEXT
├── api_endpoint TEXT
├── auth_type TEXT (bearer|api_key|header|none)
├── auth_token_encrypted TEXT (AES-256-GCM)
├── health_endpoint TEXT
├── cost_per_operation_usd NUMERIC
├── timeout_ms INTEGER
...
engine_swaps (ALTERED)
├── is_external BOOLEAN (NEW)
└── external_engine_id UUID FK → external_engines (NEW)
```

### 10.5 Admin UI

- **`/admin/governor/engines`** — Register, test, and manage external engines
- **`/admin/governor/swaps`** — Enhanced with Internal/External toggle
- **`/admin/governor`** — External Engines button added to header

### 10.6 API Routes (6 new endpoints)

| Route | Method | Purpose |
|-------|--------|---------|
| `/api/admin/governor/external-engines` | GET | List all external engines |
| `/api/admin/governor/external-engines` | POST | Register new external engine |
| `/api/admin/governor/external-engines` | PATCH | Update external engine |
| `/api/admin/governor/external-engines` | DELETE | Deactivate external engine |
| `/api/admin/governor/external-engines/test` | POST | Test connectivity |
| `/api/admin/governor/swaps` | POST | Extended: supports `isExternal` + `externalEngineId` |

### 10.7 Implementation Tasks (Phase 6)

| # | Task | Files | Status |
|---|---|---|---|
| G-038 | Define ExternalEngineRecord types | `governor/types.ts` | ✅ DONE |
| G-039 | Create external_engines migration | `033_external_engines.sql` | ✅ DONE |
| G-040 | Build external adapter | `governor/external-adapter.ts` | ✅ DONE |
| G-041 | Enhance dispatch for external routing | `governor/dispatch.ts` | ✅ DONE |
| G-042 | External engines CRUD API | `api/admin/governor/external-engines/` | ✅ DONE |
| G-043 | External engines test API | `api/admin/governor/external-engines/test/` | ✅ DONE |
| G-044 | Update swaps API for external | `api/admin/governor/swaps/` | ✅ DONE |
| G-045 | Admin external engines page | `admin/governor/engines/page.tsx` | ✅ DONE |
| G-046 | Enhanced swap page UI | `admin/governor/swaps/page.tsx` | ✅ DONE |
| G-047 | Governor dashboard update | `admin/governor/page.tsx` | ✅ DONE |

**Total: 47 atomic tasks across 6 phases**
...
>
> Last updated: 2026-03-27 — Added Section 10 (External Engine API Integration)
