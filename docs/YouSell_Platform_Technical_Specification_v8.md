...
## Appendix N — External Engine API Integration (NEW v8.1, 2026-03-27)

### N.1 Overview

The Engine Governor now supports **external engine APIs**. Any third-party
platform that provides an API can register as a replacement for any of the
25 internal engines. This enables:

- A/B testing between internal and external engine implementations
- Gradual migration to specialized external services
- Redundancy: swap to external if internal engine degrades
- Cost optimization: use cheaper external alternatives

### N.2 Architecture

```
Admin registers external engine
    ↓
external_engines table (encrypted auth tokens)
    ↓
Admin creates swap (source_engine → external_engine)
    ↓
engine_swaps table (is_external=true, external_engine_id=UUID)
    ↓
Governor Dispatch refreshes cache (30s TTL)
    ↓
Incoming request for swapped engine
    ↓
callExternalEngine() → HTTP POST to api_endpoint
    ↓
Response normalized → EngineOperationResult
    ↓
Metered in engine_usage_ledger (same as internal)
```

### N.3 Database Tables

**`external_engines`** — Registry of external engine APIs
- `engine_key` (UNIQUE slug), `display_name`, `api_endpoint`
- `auth_type` (bearer/api_key/header/none), `auth_token_encrypted` (AES-256-GCM)
- `health_endpoint`, `cost_per_operation_usd`, `timeout_ms`
- `replaces_engine` (which internal engine this can replace)
- `active`, `last_health_check`, `last_health_status`, `metadata` (JSONB)

**`engine_swaps`** — Extended with:
- `is_external` BOOLEAN — whether target is external
- `external_engine_id` UUID FK → external_engines

### N.4 API Endpoints

| Endpoint | Methods | Purpose |
|----------|---------|---------|
| `/api/admin/governor/external-engines` | GET, POST, PATCH, DELETE | CRUD for external engines |
| `/api/admin/governor/external-engines/test` | POST | Test connectivity |
| `/api/admin/governor/swaps` | POST | Extended: `isExternal` + `externalEngineId` |

### N.5 Admin UI

- `/admin/governor/engines` — Register, test, quick-swap external engines
- `/admin/governor/swaps` — Internal/External toggle for swap creation
- `/admin/governor` — External Engines button in header

### N.6 Security

- Auth tokens encrypted at rest (AES-256-GCM via `crypto.ts`)
- Tokens never returned to frontend (masked as `••••••••`)
- Admin-only RLS policies on `external_engines` table
...
| File | Purpose |
|------|---------|
| `src/lib/engines/governor/types.ts` | `ExternalEngineRecord` interface |
| `src/lib/engines/governor/external-adapter.ts` | HTTP adapter |
| `src/lib/engines/governor/dispatch.ts` | Dual-cache dispatch |
| `src/app/api/admin/governor/external-engines/route.ts` | CRUD API |
| `src/app/api/admin/governor/external-engines/test/route.ts` | Test API |
| `src/app/admin/governor/engines/page.tsx` | Registration UI |
| `supabase/migrations/033_external_engines.sql` | DB migration |

---

**END OF YOUSELL PLATFORM TECHNICAL SPECIFICATION v8.1**
