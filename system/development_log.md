...
## 2026-03-27 — External Engine API Integration

### Feature: External Engine Registration & Swap System

Added the ability for any external platform with an API to register as an
engine replacement in the YOUSELL Governor system. Any of the 25 internal
engines can now be swapped with an external API endpoint at runtime.

### Architecture Changes

**New files created:**
- `supabase/migrations/033_external_engines.sql` — New `external_engines` table + `engine_swaps` alterations
- `src/lib/engines/governor/external-adapter.ts` — HTTP adapter (auth, circuit breaker, timeout)
- `src/app/api/admin/governor/external-engines/route.ts` — CRUD API (GET/POST/PATCH/DELETE)
- `src/app/api/admin/governor/external-engines/test/route.ts` — Connectivity test API
- `src/app/admin/governor/engines/page.tsx` — External engine registration & management UI

**Modified files:**
- `src/lib/engines/governor/types.ts` — Added `ExternalEngineRecord`, `ExternalAuthType`
- `src/lib/engines/governor/dispatch.ts` — Dual cache (internal + external), external routing
- `src/app/api/admin/governor/swaps/route.ts` — External swap support (`isExternal`, `externalEngineId`)
- `src/app/admin/governor/swaps/page.tsx` — Internal/External toggle, external engine dropdown
- `src/app/admin/governor/page.tsx` — External Engines button in header

### Key Design Decisions
- External engines communicate via HTTP POST only (no EventBus access)
- Auth tokens encrypted with AES-256-GCM before storage
- Circuit breaker per external engine (3 failures → open)
- Dispatch cache refreshes every 30s (same as internal swaps)
- External engine health checks via configurable health endpoint

### Metrics Update
- Admin pages: 44 → 45 (+1 external engines page)
- API routes: 97 → 99 (+2 external engine routes)
- Database migrations: 32 → 33 (+1 external engines migration)
- Governor tasks: 37 → 47 (+10 external engine tasks)

------------------------------------------------------------

## 2026-03-27 — AI Intelligence Pages (Aalpha Alignment)

### Feature: 5 AI Intelligence Admin Pages + API Routes + Migration

Added 5 new admin pages aligned with Aalpha AI Ecommerce Intelligence categories,
bringing AI-native intelligence management directly into the admin dashboard.

### New Files Created

**Admin Pages (5):**
- `src/app/admin/chatbot/page.tsx` — Chatbot AI management (intents, conversations, bot config)
- `src/app/admin/fraud/page.tsx` — Fraud & Security monitoring (transaction monitoring, risk detection, fraud rules)
- `src/app/admin/pricing/page.tsx` — Dynamic Pricing intelligence (AI pricing, competitor price tracking)
- `src/app/admin/forecasting/page.tsx` — Demand Forecasting (inventory prediction, stockout alerts, restock management)
- `src/app/admin/smart-ux/page.tsx` — Smart UX management (AI frontend feature toggles, A/B testing, personalization rules)

**API Routes (5):**
- `src/app/api/admin/chatbot/route.ts` — Chatbot config, intents, conversations CRUD
- `src/app/api/admin/fraud/route.ts` — Fraud rules, fraud flags management
- `src/app/api/admin/pricing/route.ts` — Pricing strategies, suggestions, competitor prices
- `src/app/api/admin/forecasting/route.ts` — Demand forecasts, restock alerts
- `src/app/api/admin/smart-ux/route.ts` — Smart UX features, A/B tests, personalization rules

**Migration (1):**
- `supabase/migrations/034_ai_intelligence_tables.sql` — 13 new tables:
  - Chatbot: `chatbot_config`, `chatbot_intents`, `chatbot_conversations`
  - Fraud: `fraud_rules`, `fraud_flags`
  - Pricing: `pricing_strategies`, `pricing_suggestions`, `competitor_prices`
  - Forecasting: `demand_forecasts`, `restock_alerts`
  - Smart UX: `smart_ux_features`, `ab_tests`, `personalization_rules`

### Key Design Decisions
- Each page follows existing admin page patterns (EnginePageLayout, authFetch, pagination)
- API routes use requireAdmin() auth guard consistently
- 13 new tables follow existing RLS patterns (admin full access, service role bypass)
- All pages added to admin sidebar under new "AI Intelligence" nav section
