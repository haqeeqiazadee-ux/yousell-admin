...
## Version 9.0 — Unified Master Build Brief

**Document Status:** CANONICAL — This document supersedes ALL prior build briefs (v4, v5, v6, v7, v8) and is the single authoritative architecture reference for the YouSell Platform. It is the product of a full reconciliation merge of v7 and v8.
...
**What Changed v8 → v9:** Full reconciliation merge of v7 (2,683 lines) and v8 (4,310 lines) into one definitive specification. Every section, table, schema, formula, bug reference, API route, queue definition, appendix, and business rule from both documents has been verified as present. No content was dropped. v7-specific historical context (v6→v7 changelog) restored to Appendix H. Affiliate commission estimates carry forward v8's corrected rates with ⚠️ verification warnings. All directory path references standardised to `/system` (v8 convention, replacing v7's `/ai`). Phase references use letter convention (A–L) per v8. This document is approximately 4,400+ lines and is self-contained — no external document is required to understand the full platform architecture.

**What Changed v7 → v8 (preserved for historical context):** Full integration of Print on Demand (Channel #8), Admin Command Center (Best-Selling Products Dashboard), Affiliate Commission Engine (Dual Revenue Tracking), 8 new BullMQ queues (23 total), expanded content strategy, competitive landscape analysis, POD fulfillment partner research, affiliate program research (80+ sources), updated development phases, revenue multiplier projections, and fulfillment recommendation logic. All satellite documents merged into a single authoritative specification.
...
6. **Revenue multiplier quantified** — ~$95K/yr estimated at 50 clients from affiliate commissions alone (corrected from $124K after Jasper removal + rate verification).

**From v8 to v9 — what changed:**

1. **Full reconciliation merge** — Every section from v7 and v8 verified present. Zero content loss.
2. **v6→v7 historical changelog restored** — Previously only v7→v8 changes were documented.
3. **Affiliate revenue estimate corrected** — $95K/yr (v8 corrected) with ⚠️ verification warnings preserved.
4. **Directory path standardised** — `/system` convention (v8) confirmed over v7's `/ai`.
5. **All internal document references updated** — Now point to v9.
...
1. **Read these files in order:**
   ...
   - `docs/YouSell_Platform_Technical_Specification_v9.md` — This document (canonical architecture)
...
8. **Treat this v9 specification as the primary architecture reference** unless a newer canonical file explicitly replaces it.
...
|---|---|---|
| `docs/YouSell_Platform_Technical_Specification_v9.md` | Master architecture specification | #1 — Highest |
...
| File | Status | Reason |
|---|---|---|
| `docs/YouSell_Platform_Technical_Specification_v8.md` | Superseded | Replaced by this v9 specification |
| `docs/YouSell_Platform_Technical_Specification_v7.md` | Superseded | Replaced by this v9 specification |
| `YouSell_BuildBrief_v6_DEFINITIVE.docx` | Superseded | Replaced by v7, then v8, now v9 |
| `YOUSELL_MASTER_BUILD_BRIEF_v4.pdf` | Superseded | Five versions behind |
| `YOUSELL_MASTER_BUILD_BRIEF_v5.pdf` | Superseded | Four versions behind |
...
If any file conflicts with this v9 specification, the v9 specification takes precedence. If an even newer file is created, it must explicitly declare itself as the new canonical source and reference this v9 document.
...
```
1. Read CLAUDE.md
2. Read docs/YouSell_Platform_Technical_Specification_v9.md (this document)
...
| v7 | 2026-03-13 | Full reconciliation: Stripe, store integrations, content engine, order tracking, engine toggles, platform gating, corrected pricing, affiliate business model, QA audit (41 bugs), dual-platform separability, OAuth channels. **v6→v7 specific changes:** (1) Stripe payment integration added, (2) Store integration APIs added (Shopify GraphQL, TikTok Seller, Amazon SP-API), (3) OAuth channel integrations for automated content distribution, (4) Order tracking via Resend + Railway, (5) Pricing corrected from $2,997 agency pricing to $29–$299/mo SaaS, (6) Dual-platform architecture defined (admin.yousell.online + yousell.online), (7) Engine toggle system designed |
| **v8** | **2026-03-17** | **Merged all satellite documents. Added: POD Channel #8 (full integration), Admin Command Center (Best-Selling Products Dashboard), Affiliate Commission Engine (dual revenue tracking), 8 new BullMQ queues (23 total), 7 new database tables, 15 new API routes, POD fulfillment partner APIs (Printful/Printify/Gelato), expanded content strategy, competitive landscape analysis (POD + affiliate tiers), market research (80+ sources integrated), revenue multiplier projections ($95K/yr corrected), fulfillment recommendation logic, updated development phases (J/K/L), 20-22 week timeline** |
| **v9** | **2026-03-20** | **Full reconciliation merge of v7 (2,683 lines) and v8 (4,310 lines). Verified every section, table, schema, formula, bug, API route, queue, and appendix present. No content dropped. v7 historical changelog restored. Affiliate rates carry v8 corrections with ⚠️ warnings. Directory paths standardised to /system. Phase letter convention (A–L) confirmed. Self-contained ~4,400+ line definitive specification.** |
...
1. `docs/YouSell_Platform_Technical_Specification_v9.md` — THIS DOCUMENT — Master architecture (supersedes v7 and v8)
...
**END OF YOUSELL PLATFORM TECHNICAL SPECIFICATION v9.0**

*This document is the single authoritative architecture reference for the YouSell Platform. It supersedes v7, v8, and all prior build briefs. It is the product of a full reconciliation merge — every section, table, schema, formula, bug reference, API route, queue definition, appendix, and business rule from both v7 and v8 has been verified as present. All development, continuity recovery, and architectural decisions should reference this document first.*
